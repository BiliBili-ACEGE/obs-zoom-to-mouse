...
function script_properties()
    ...
    local sources_list = obs.obs_properties_add_list(props, "source", "选择缩放源", obs.OBS_COMBO_TYPE_LIST,
    ...
    local follow = obs.obs_properties_add_bool(props, "follow", "自动跟随鼠标指针 ")
    obs.obs_property_set_long_description(follow,
        "启用后，鼠标跟踪将在放大时自动启动，无需等待跟踪切换热键")

    local follow_outside_bounds = obs.obs_properties_add_bool(props, "follow_outside_bounds", "始终跟随指针")
    obs.obs_property_set_long_description(follow_outside_bounds,
        "启用后，即使光标位于缩放源的边界之外，也会跟踪鼠标")
    ...
    obs.obs_property_set_long_description(follow_auto_lock,
        ...
        "但是向后移动到中心将停止跟踪 RTS 游戏中的平移镜头")

    local allow_all = obs.obs_properties_add_bool(props, "allow_all_sources", "允许任何缩放源 ")
    obs.obs_property_set_long_description(allow_all, "启用此选项可允许选择任何源作为缩放源\n" ..
        "您必须为非显示捕获源设置手动源位置")
    ...
    obs.obs_property_set_long_description(override,
        "启用后，指定的大小/位置的设置将用于缩放源，而不是自动计算的设置")
    ...
    obs.obs_property_set_long_description(debug,
        "启用后，脚本会将诊断消息输出到脚本日志（对于调试/github 问题很有用）")
...
function script_load(settings)
    ...
    -- Add our hotkey
    hotkey_zoom_id = obs.obs_hotkey_register_frontend("toggle_zoom_hotkey", "在鼠标指针处缩放",
        on_toggle_zoom)

    hotkey_follow_id = obs.obs_hotkey_register_frontend("toggle_follow_hotkey", "跟随鼠标指针缩放",
        on_toggle_follow)
